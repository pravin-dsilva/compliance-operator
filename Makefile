...
DEFAULT_CONTENT_IMAGE_PATH=quay.io/abonnet/ocp4:latest
E2E_CONTENT_IMAGE_PATH?=quay.io/abonnet/ocp4:latest
# We specifically omit the tag here since we only use this for testing.
E2E_BROKEN_CONTENT_IMAGE_PATH?=quay.io/abonnet/test-broken-content:latest
...
MUST_GATHER_IMAGE_PATH?=quay.io/abonnet/must-gather
...
test-catalog: index-image-to-cluster
	...
	@$(SED) 's%quay.io/abonnet/compliance-operator-index:latest%$(LOCAL_INDEX_IMAGE_PATH)%' deploy/olm-catalog/catalog-source.yaml
	...
	@$(SED) 's%$(LOCAL_INDEX_IMAGE_PATH)%quay.io/abonnet/compliance-operator-index:latest%' deploy/olm-catalog/catalog-source.yaml
...
openshift-user:
ifeq ($(shell oc whoami 2> /dev/null),system:admin)
